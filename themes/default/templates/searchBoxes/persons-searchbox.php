...
<?php else: ?>	
	...
	<nav class="searchbox hidden-print" >
		<form id="searchForm" class="searchForm" method="get" action="<?= $this->buildUrl('persons/results')?>s" name="searchForm" autocomplete="off">
...
AC(document.getElementById("searchForm_lookfor"), function achandler(query, callback) {
    const url = "<?= $this->buildUrl('autocomplete/persons.json',['lookfor'=>null, 'type'=>null])?>q="+query;
    fetch(url).then(function(response) {
        return response.json();
    }).then(function(json) {
        callback( json.searchResults.map( persons => ({
            text: persons.name,
			sub: persons.year_born+'-'+persons.year_death
        })  ) );
    });
});
